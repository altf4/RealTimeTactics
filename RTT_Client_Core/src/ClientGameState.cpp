		...
		struct Coordinate destination, enum Direction facing)
{
	...
	if((movingUnit.x != source.x) || (movingUnit.y != source.y))
	{
	...
	movingUnit.x = destination.x;
	movingUnit.y = destination.y;
	movingUnit.directionFacing = facing;
